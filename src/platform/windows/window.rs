    ...
    Win32::{
        ...
        Graphics::{
            ...
            OpenGL::{
                ...
                GL_COLOR_BUFFER_BIT, GL_TRUE, HGLRC, PFD_DOUBLEBUFFER, PFD_DRAW_TO_WINDOW,
                PFD_MAIN_PLANE, PFD_PIXEL_TYPE, PFD_SUPPORT_OPENGL, PFD_TYPE_RGBA,
...
// Window data --> common to all windows window
...
#[cfg(feature = "opengl")]
#[derive(Debug)]
pub struct OpenGLWindowsWindow {
    window_handle: HWND,
    dc: HDC,
    context: HGLRC,
    window_data: Box<WindowData>,
...
impl WindowImpl for OpenGLWindowsWindow {
    type WindowImplType = OpenGLWindowsWindow;
    fn create() -> OpenGLWindowsWindow {
        OpenGLWindowsWindow::new_opengl_window()
    }
...
impl OpenGLWindowsWindow {
    fn new_opengl_window() -> OpenGLWindowsWindow {
        ...
        let _temporary_window = OpenGLWindowsWindow::create_window_and_context(
            w!("Temporary window class"),
            w!("Temporary window"),
            choose_pixel_format,
            Some(OpenGLWindowsWindow::temp_wndproc),
        );
        ...
        let window = OpenGLWindowsWindow::create_window_and_context(
    ...
    fn create_window_and_context(
        ...
        let window_handle =
            create_window(instance, window_class, window_name, wnd_proc, &window_data);

        let dc = unsafe { GetDC(window_handle) };

        set_pixel_format(dc, get_pixel_fmt);

        let context = OpenGLWindowsWindow::create_openl_context(dc);
...
impl Drop for OpenGLWindowsWindow {
    fn drop(&mut self) {
